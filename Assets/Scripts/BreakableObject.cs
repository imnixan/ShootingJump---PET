...
[AddComponentMenu("Breakable objects/Breakable object")]
...
public class BreakableObject : MonoBehaviour
{
    ...
    private void bakeVertices(bool trianglesToo = false)
    {
        ...
        for (int y = 0; y < partsY + 1; y++)
        {
            ...
            {
                float randomX =
                    Random.value > 0.5f ? Random.value / partsX : -Random.value / partsX;
                float randomY =
                    Random.value > 0.5f ? Random.value / partsY : -Random.value / partsY;
                vertices[y * (partsX + 1) + x] = new Vector3(
                    (float)x / (float)partsX - 0.5f + randomX,
                    (float)y / (float)partsY - 0.5f + randomY,
                    0
                );
    ...
    private void generateSingleSplinter(int[] tris, Transform parent)
    {
        ...
        obj.transform.position = new Vector3(
            vertices[tris[0]].x * transform.localScale.x + transform.position.x,
            vertices[tris[0]].y * transform.localScale.y + transform.position.y,
    ...
    private void bakeSplinters()
    {
        ...
        if (preCalculate)
            splinterParent.SetActive(false);
    ...
    void OnCollisionEnter(Collision col)
    {
        if (useCollision == true)
        {
            if (health > 0)
            {
                ...
                {
                    health = 0;
                    BreakObject(col.GetContact(0).point);
                }
            }
            else
                BreakObject(col.GetContact(0).point);
